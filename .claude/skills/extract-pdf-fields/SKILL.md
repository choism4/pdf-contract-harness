...
- 표 격자선은 기본으로 제외한다(가로선 내부를 세로선이 관통하거나 양끝에 세로선=닫힌 셀 → 표 테두리로 판단). `--keep-table-lines`로 끌 수 있다.
- **셀에 직접 채우는 폼**(예: 중개대상물 확인설명서)은 격자선 제거 시 칸이 사라진다 — 빈 셀 자체를 필드로 만드는 셀 탐지는 미구현(후속). 이런 문서는 웹앱에서 `+ Add field`로 칸을 그리거나 `--keep-table-lines`로 선을 살린다.
