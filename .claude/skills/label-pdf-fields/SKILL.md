...
description: Semantically label the extracted fill-in fields in a contract's fields.json — fill each field's label/fill_hint, fix its type (e.g. signature/seal), and mark confirmed. Use after extract-pdf-fields has produced fields.json, when the user asks to label/name/refine fields, interpret what each blank is for, or prepare a contract for export. This is the "brain" step; it edits fields.json directly (no script, no web app needed).
---
...
- 너가 바꾸는 필드: `label`, `fill_hint`, `type`, `status`. 좌표를 손댔다면 잘못한 것이다.
...
- `(서명)`, `(인)`, `날인`이 줄에 있으면 → `type:"signature_seal"`.
- 한 줄에 여러 라벨(`이름 : 주민등록번호 : 연락처 :`)이면 각 빈칸의 `bbox` x좌표로 어느 라벨에 붙는지 구분(추출이 이미 처리, 검증만).

## 다음 단계

라벨이 `confirmed`되면 익스포트(Phase 5): `export.json` 생성 → 다운스트림 프로젝트의 Claude가 소비. 익스포트는 웹앱 "익스포트" 버튼 또는 `app/export.py` CLI로.
